...
export default function ManageOrderTable() {
  ...
  const filter = (timeFilter) => {
    ...
    axiosClient
      .get(
        `/orders?page=${currentPage}&perPage=${ordersPerPage}&orderBy=${timeFilter}`
      )
      .then((response) => {
        setOrders(response.data.orders);
        console.log(orders);
  ...
  return (
    <div className="parent">
      {showAlert && (
        <div
          id="alert-border-2"
          className="flex fixed z-40 justify-top transition-transform bottom-100 top-1 left-100 right-1 w-max p-4 mb-4 text-red-800 border-b-4 border-red-400 bg-red-50"
          role="alert"
        >
          <svg
      ...
      <div className="relative overflow-x-auto sm:rounded-lg">
        <div className="bg-white rounded-md backdrop-filter backdrop-blur-lg bg-opacity-95">
                  ...
                  <tr key={order.id} className="bg-white">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        type="submit"
                        className="font-medium bg-gray-50 p-3 rounded-md hover:bg-gray-200 text-gray-500"
                      >
                        Modify
                      </button>
                    </td>
