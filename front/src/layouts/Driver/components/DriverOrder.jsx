...
export default function DriverOrder() {
  ...
  useEffect(() => {
    ...
    axiosClient
      .get(`/driverorders?page=${page}&perPage=10&driver_id=${currentUser.id}`)
      .then((response) => {
  ...
  const paginate = (pageNumber) => {
    ...
    axiosClient
      .get(`/driverorders?page=${pageNumber}&perPage=${ordersPerPage}`)
      .then((response) => {
  ...
  const getStatusTableColor = (status) => {
    switch (status) {
      case "cancelled":
        return <div className="flex bg-red-400 h-20"></div>;
      case "delivering":
        return <div className="flex bg-green-200 h-20"></div>;
      case "delivered":
        return <div className="flex bg-green-500 h-20"></div>;
    ...
    return (
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div className="grid items-start justify-start">
          <table className="grid grid-cols-4 w-full text-sm text-left text-gray-500">
    ...
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <div className="grid items-start justify-start">
        <table className="grid grid-cols-2 w-full text-sm text-left text-gray-500">
          <thead className="border-r-2 text-xs border-gray-100 uppercase bg-white">
            <tr className="grid grid-cols-1 justify-start bg-white">
              <th scope="col" className="ml-2 text-black">
                Driver:{" "}
          ...
          <tbody className="bg-white flex flex-row">
            {orders && orders.length > 0 ? (
              orders.map((order) => (
                <tr key={order.id} className="flex flex-col bg-white">
                  <input
                    type="hidden"
                    name="employee_id"
                    value={currentUser.id}
